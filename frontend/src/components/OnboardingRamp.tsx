        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {['workspace', 'project', 'done'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              {i < 2 && <div className={`w-12 h-0.5 ${['workspace', 'project', 'done'].indexOf(step) > i ? 'bg-green-500' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
        ...
        {step === 'workspace' && (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mt-2">A workspace is your team&apos;s home — it contains projects, members, and your state machine.</p>
            </div>
            <form onSubmit={handleCreateWorkspace} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-0 border border-gray-300 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                  <span className="px-3 py-3 bg-gray-50 text-sm text-gray-400 border-r border-gray-300">openweave.dev/</span>
                  <input
                    value={wsSlug}
                    onChange={e => setWsSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                    className="flex-1 px-3 py-3 text-sm border-0 focus:ring-0 outline-none"
                    placeholder="my-team"
                  />
                </div>
              </div>
              <button type="submit" disabled={saving || !wsName.trim()} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors">
        ...
        {step === 'project' && (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mt-2">Projects group tickets and team members. You can create more later.</p>
            </div>
            <form onSubmit={handleCreateProject} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={10}
                />
                <p className="text-xs text-gray-400 mt-1">Used in ticket IDs like {projSlug || 'WR'}-1, {projSlug || 'WR'}-2</p>
              </div>
              <div>
              ...
              <button type="submit" disabled={saving || !projName.trim()} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors">
