    ...
    <Layout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
        ...
        <div className="space-y-3">
          {workspaces.map(ws => (
            <button key={ws.slug} onClick={() => handleSelect(ws)} className="w-full bg-[#111118] border border-[#222233] rounded-xl p-5 hover:shadow-md transition-shadow text-left">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="font-semibold text-white">{ws.name}</h2>
                  <p className="text-sm text-gray-400">/{ws.slug} · {ws.member_count} member{ws.member_count !== 1 ? 's' : ''}</p>
                </div>
