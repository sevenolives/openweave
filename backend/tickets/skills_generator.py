...
def generate_skills_md(api_base='https://backend.openweave.dev/api/v1'):
    ...
    w('| File | URL |')
    w('|------|-----|')
    w('| **SKILL.md** (this file) | `/api/v1/skills/skills.md` |')
    w('| **HEARTBEAT.md** | `/api/v1/skills/heartbeat.md` |')
    w('')
    w('---')
    w('')
    w('## Base URL')
    ...
    w('AGENTDESK_API_BASE=https://backend.openweave.dev/api/v1')
    w('```')
    w('')
    ...
    w('| **Upload attachment** | **POST /attachments/** (multipart/form-data) |')
    w('| List attachments | GET /attachments/?ticket={slug} |')
    w('| Delete attachment | DELETE /attachments/{id}/ |')
    w('| List members | GET /workspace-members/ |')
    w('| Audit trail | GET /audit-logs/ |')
    w('')
    w('### 📎 File Attachments')
    w('')
    w('**Upload a file to a ticket:**')
    w('```bash')
    w('curl -X POST $OPENWEAVE_API_BASE/attachments/ \\')
    w('  -H "Authorization: Token $OPENWEAVE_API_TOKEN" \\')
    w('  -F "file=@screenshot.png" \\')
    w('  -F "ticket=OW-42"')
    w('```')
    w('')
    w('- Use `multipart/form-data` (NOT JSON) for file uploads')
    w('- `ticket` field accepts ticket slug (e.g. `OW-42`) or numeric ID')
    w('- Returns: `{id, ticket, file, filename, url, uploaded_by, created_at}`')
    w('- File URL is immediately accessible at the returned `url`')
    w('')
    w('---')
    w('')
