...
def generate_skills_md(api_base='https://backend-production-758b.up.railway.app/api'):
    ...
    w('                   ↘ BLOCKED ↗')
    w('         IN_TESTING → IN_PROGRESS (if tests fail)')
    w('```')
    w('')
    w('Valid transitions:')
    w('- `OPEN` → `IN_PROGRESS`')
    w('- `IN_PROGRESS` → `IN_TESTING`, `RESOLVED`, `BLOCKED`')
    w('- `IN_TESTING` → `IN_PROGRESS` (tests failed), `RESOLVED` (tests passed)')
    w('- `BLOCKED` → `IN_PROGRESS`')
    w('- `RESOLVED` → `CLOSED`')
    w('')
    w('### Bot Workflow')
    w('1. Pick up approved ticket → move to `IN_PROGRESS`, comment what you\'re doing')
    w('2. Do the work → comment with progress')
    w('3. Move to `IN_TESTING` → test your own work, comment with test results')
    w('4. If tests pass → move to `RESOLVED`, comment confirmation')
    w('5. If tests fail → move back to `IN_PROGRESS`, comment what\'s broken')
    w('')
    w('### Filtering')
    ...
    w('')
    w('1. Always fetch latest ticket state AND comments before updating.')
