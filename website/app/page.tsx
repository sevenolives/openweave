...
function useReveal() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([e]) => { if (e.isIntersecting) { el.classList.add("fade-in"); obs.unobserve(el); } },
      { threshold: 0.15 }
    );
    obs.observe(el);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <span className="text-xl font-bold text-brand-600">Agent Desk</span>
          <a href={APP} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 transition">
            Open App →
          </a>
        </div>
      </nav>
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-brand-600 via-brand-700 to-brand-900 text-white">
        ...
        <div className="relative max-w-4xl mx-auto px-4 py-24 md:py-36 text-center">
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight leading-tight fade-in">
            AI-Powered Support,<br />Human-Level Care
          </h1>
          <p className="mt-6 text-lg md:text-xl text-indigo-100 max-w-2xl mx-auto fade-in" style={{ animationDelay: ".15s" }}>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center fade-in" style={{ animationDelay: ".3s" }}>
            <a href={APP} className="inline-flex items-center justify-center rounded-xl bg-white text-brand-700 font-semibold px-8 py-3.5 hover:bg-indigo-50 transition shadow-lg shadow-brand-900/30">
              Get Started Free
            </a>
          ...
          <div className="mt-14 grid gap-8 md:grid-cols-3">
            {steps.map((s) => (
              <div key={s.num} className="flex flex-col items-center">
                <span className="flex items-center justify-center w-14 h-14 rounded-full bg-brand-100 text-brand-700 text-xl font-bold">{s.num}</span>
      ...
      {/* CTA */}
      <Section className="py-20 md:py-28 bg-gradient-to-r from-brand-600 to-brand-700 text-white text-center">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <a href={APP} className="mt-8 inline-flex items-center justify-center rounded-xl bg-white text-brand-700 font-semibold px-8 py-3.5 hover:bg-indigo-50 transition shadow-lg">
            Launch Agent Desk →
          </a>
        </div>
      </Section>
